...
int avpriv_tempfile(const char *prefix, char **filename, int log_offset, void *log_ctx)
{
    ...
    int fd = -1;
#if HAVE_MKSTEMP
    size_t len = strlen(prefix) + 12; /* room for "/data/data/com.termux/files/usr/tmp/" and "XXXXXX\0" */
...
#else
    snprintf(*filename, len, "/data/data/com.termux/files/usr/tmp/%sXXXXXX", prefix);
    fd = mkstemp(*filename);
